...
1. `libtrn/trn.cpp`, `get_newsgroup_dir`

   Promote the parameter from `const char *` to `std::string_view`.
   The helper immediately copies the input into `std::string` and only
   rewrites dots to slashes.  Update the single caller in
   `set_newsgroup_name` to pass `g_newsgroup_name` directly.

2. `util/env.cpp`, `export_var`
...
3. `libtrn/ngsrch.cpp`, `newsgroup_comp`
...
4. `libtrn/autosub.cpp`, `match_list`
...
7. `libtrn/univ.cpp`, `univ_use_file`
...
8. `libtrn/edit_dist.cpp`, `edit_distn`
...
9. `libtrn/cache.cpp`, `decode_header`
...
10. `libtrn/cache.cpp`, `Article::set_subj_line`
...
12. `libtrn/datasrc.cpp`, `DataSource::find_active_group`
