...
static void sel_page_init()
{
    ...
    g_sel_chars = get_env_var("SELECTCHARS", SELECTION_CHARS);
...
try_again:
    ...
    case SM_NEWSGROUP:
    {
        ...
        for (NewsgroupData *np = newsgroup_first(); np; np = newsgroup_next(np))
        {
            ...
            else
            {
                ...
                if (!np->m_rc->data_source->m_act_sf.m_fp)
                {
                    save_the_rest = (g_sel_rereading ^ (np->m_to_read > TR_NONE));
                }
            }
            if (g_paranoid)
            {
                ...
                cleanup_newsrc(np->m_rc);
                goto try_again;
            }
            if (!(np->m_flags & g_sel_mask)
             && (g_sel_rereading? np->m_to_read != TR_NONE
                              : np->m_to_read < g_newsgroup_min_to_read))
            {
                continue;
            }
            ++g_obj_count;

            if (!g_sel_exclusive || (np->m_flags & g_sel_mask))
            {
                if (np->m_flags & g_sel_mask)
                {
                    g_selected_count++;
    ...
    case SM_ARTICLE:
    {
        ...
        if (g_sel_page_app)
        {
            ...
            if (ap == nullptr)
            {
                g_sel_page_app = article_ptr_list_end();
            }
            else
            {
                limit = article_ptr_list_end();
                for (app = article_ptr_list_begin(); app < limit; app++)
                {
                    if (*app == ap)
                    {
                        g_sel_page_app = app;
...
bool next_page()
{
    ...
    case SM_ARTICLE:
    {
        if (g_sel_next_app < article_ptr_list_end())
        {
...
bool prev_page()
{
    ...
    case SM_ARTICLE:
    {
        ...
        for (Article **app = g_sel_page_app; --app >= article_ptr_list_begin();)
        {
            if ((*app)->m_flags & AF_INCLUDED)
            {
                page_app = app;
...
start_of_loop:
        for (np = g_sel_page_np; np; np = newsgroup_next(np))
        {
            ...
            if (outputting)
            {
                sel = !!(np->m_flags & g_sel_mask) + (np->m_flags & NF_DEL);
...
void update_page()
{
    ...
    for (int j = 0; j < g_sel_page_item_cnt; j++)
    {
        ...
        case SM_NEWSGROUP:
            sel = !!(u.np->m_flags & g_sel_mask) + (u.np->m_flags & NF_DEL);
            break;
...
static void display_article(const Article *ap, int ix, int sel)
{
    ...
    {
        fmt::print("{}  {}\n", compress_from(ap->from_view(), from_width), compress_subj(ap, subj_width - from_width));
    }
    term_down(1);
}
...
static void display_subject(const Subject *subj, int ix, int sel)
{
    ...
    else
    {
        ...
        {
            fmt::print("{}{:3}  {}\n", compress_from(first_ap ? first_ap->from_view() : std::string_view{}, from_width),
                       j, compress_subj(first_ap, subj_width - from_width));
        }
        term_down(1);
        int i = -1;
            ...
            for (; ap && j; ap = ap->m_subj_next)
            {
                ...
                else if (current_article_display_mode() == 'm')
                {
                    ...
                    else
                    {
                        ...
                        fmt::print("  {}      ", compress_from(ap ? ap->from_view() : std::string_view{}, from_width));
                        continue;
                    }
                }
                if (g_term_line >= s_sel_max_line_cnt + 2)
                ...
                fmt::print("  {}\n", compress_from(ap ? ap->from_view() : std::string_view{}, from_width));
                term_down(1);
            }
        }
    }
