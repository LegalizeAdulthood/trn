...
int help_page()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("Paging commands:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("b	Back up one page.\n"
                           "^E	Display the last page of the article.\n"
                           "^L,X	Refresh the screen (X=rot13).\n"
                           "_C      Switch characterset conversion.\n",
                           NOMARKING)) ||
        (cmd = print_lines("t	Display the entire article tree and all its subjects.\n", NOMARKING)) ||
        (cmd = print_lines("g pat	Go to (search forward within article for) pattern.\n"
                           ...
                           "\n",
                           NOMARKING)) ||
        (cmd = print_lines("The following commands skip the rest of the current article, then behave\n"
        ...
        (cmd = print_lines("\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("<, >	Browse the previous/next selected thread.  If no threads are selected,\n"
                           ...
                           "\n",
                           NOMARKING)) ||
        (cmd = print_lines(";	Enter article-scan mode.\n", NOMARKING)) ||
        (cmd = print_lines("Scoring commands are enabled.\n"
                           "(Type h at end of article for description of Scoring commands.)\n",
                           NOMARKING)) ||
        (cmd = print_lines("The following commands also take you to the end of the article.\n"
                           "Type h at end of article for a description of these commands:\n",
                           STANDOUT)) ||
        (cmd = print_lines("	# $ & / = ? c C f F k K ^K J , m M number e r R ^R s S u U v w W Y ^ |\n"
                           "\n"
...
int help_art()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("Article Selection commands:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           "n,SP	Find next unread article (follows discussion-tree in threaded groups).\n",
                           NOMARKING)) ||
        (cmd = print_lines("N	Go to next article.\n"
        ...
        (cmd = print_lines("<, >	Browse the previous/next selected thread.  If no threads are selected,\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("number	Go to specified article.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines(" 	Valid commands are: a, e, j, m, M, s, S, t, T, |, +, ++, -, and --.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("/pattern/modifiers\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("/pattern/modifiers:command{:command}\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("f,F	Submit a followup article (F = include this article).\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("C	Cancel this article, if yours.\n"
                           ...
                           "b	Back up one page.\n",
                           NOMARKING)) ||
        (cmd = print_lines("^E	Display the last page of the article.\n"
        ...
        (cmd = print_lines("#       Print last article number.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("m	Mark article as still unread.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("J	Junk entire thread (mark all subjects as read in this thread).\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("=	List subjects of unread articles.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines(";	Enter article-scan mode.\n", NOMARKING)) ||
        (cmd = print_lines("_T	Start the thread selector if threaded, else the subject selector.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Scoring commands:\n", STANDOUT)) ||
        (cmd = print_lines("\"\" <text>\n"
...
int help_ng()
{
    ...
    if (g_ngptr)
    {
        if ((cmd = print_lines("\n"
                               ...
                               NOMARKING)) ||
            (cmd = print_lines(";	Enter article-scan mode for this newsgroup.\n", NOMARKING)) ||
            (cmd = print_lines("u	Unsubscribe from this newsgroup.\n"
    ...
    if ((cmd = print_lines("t	Toggle the newsgroup between threaded and unthreaded reading.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("-	Go to the previously displayed newsgroup.\n"
                           "1	Go to the first newsgroup.\n"
                           "^	Go to the first newsgroup with unread news.\n"
                           "$	Go to the end of newsgroups.\n",
                           NOMARKING)) ||
        (cmd = print_lines("g name	Go to the named newsgroup.  Subscribe to new newsgroups this way too.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("l pat	List unsubscribed newsgroups containing pattern.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("&	Start the option selector.\n"
                           "&switch {switch}\n"
                           ...
                           "!cmd	Shell escape.\n",
                           NOMARKING)) ||
        (cmd = print_lines("`	switch to the newsgroup selector.\n"
                           "q	Quit trn.\n"
...
int help_ngsel()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("The Newsgroup Selector:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("k, ','	Mark the current group to kill all unread articles.\n"
                           "-	Set a range, as in d - f.  Repeats the last marking action.\n"
                           "@	Toggle the selection of all visible groups.\n",
                           NOMARKING)) ||
        (cmd = print_lines("E	Toggle exclusion of non-selected groups from the selection list.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("O	Set the selector's order.\n"
                           "R	Reverse the current sort order.\n"
                           "U	Switch between selecting empty/non-empty newsgroups.\n",
                           NOMARKING)) ||
        (cmd = print_lines("c	Catch up a group, marking the articles as read without chasing xrefs.\n"
                           "=	Refresh the article counts in the selector (will re-grab active).\n"
                           "^N	Switch to next news source (the numbered GROUPs in ~/.trn/access).\n"
                           "^P	Switch to the previous news source.\n"
                           "^K	Edit the global KILL file.\n",
                           NOMARKING)) ||
        (cmd = print_lines("\\	Escape next char as a command. A selector command is tried first,\n"
        ...
        (cmd = print_lines("/pattern/modifiers:command{:command}\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("&	Start the option selector or set command-line switches.\n"
...
int help_addsel()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("The Add-Newsgroups Selector:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           ...
                           ".	Toggle the current group's selection.\n",
                           NOMARKING)) ||
        (cmd = print_lines("k, ','	Mark the current group as undesirable for subscribing.\n"
                           "-	Set a range, as in d - f.  Repeats the last marking action.\n"
                           "@	Toggle the selection of all visible groups.\n",
                           NOMARKING)) ||
        (cmd = print_lines("E	Toggle exclusion of non-selected groups from the selection list.\n"
                           ...
                           "^, $	Go to first/last page.\n",
                           NOMARKING)) ||
        (cmd = print_lines("O	Set the selector's order (NOT WORKING YET).\n"
                           "R	Reverse the current sort order (NOT WORKING YET).\n",
                           NOMARKING)) ||
        (cmd = print_lines("/pattern/modifiers:commands\n"
...
int help_subs()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("Valid substitutions are:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("c	Current newsgroup, directory form\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("f	Who the current article is from\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("l	News administrator's login name, if any\n"
                           "L	Login name (yours)\n",
                           NOMARKING)) ||
        (cmd = print_lines("m	Current mode, first letter of (init,newsgroup,thread,article,pager,\n"
                           "		unread,Add,Catchup,Delete-bogus,Mailbox,Resubscribe)\n",
                           NOMARKING)) ||
        (cmd = print_lines("M	Number of article marked with M\n"
                           "n	Newsgroups from current article\n"
                           "N	Full name (yours)\n"
                           "o	Organization (yours)\n"
                           "O	Original working directory (where you ran trn from)\n",
                           NOMARKING)) ||
        (cmd = print_lines("p	Your private news directory (from -d)\n"
                           "P	Public news spool directory\n"
                           "r	Last reference (parent article id)\n"
                           "R	References list for followup article\n",
                           NOMARKING)) ||
        (cmd = print_lines("s	Subject, with all Re's and (nf)'s stripped off\n"
                           "S	Subject, with one Re stripped off\n"
                           "t	New To line derived from From and Reply-To (Internet format)\n"
                           "T	New To line derived from Path\n"
                           "u	Number of unread articles\n",
                           NOMARKING)) ||
        (cmd = print_lines("U	Number of unread articles not counting the current article (when\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("y	From line with the domain abbreviated.\n"
                           "	(foo@x.y.bar.edu (Foo's Bar) -> foo@*.bar.edu (Foo's Bar))\n",
                           NOMARKING)) ||
        (cmd = print_lines("Y	The current tmp dir (usually /tmp)\n"
                           "z	Length of current article in bytes\n"
                           "Z	Number of selected threads\n",
                           NOMARKING)) ||
        (cmd = print_lines("~	Your home directory\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Put ^ in the middle to capitalize the first letter: %^C = Rec.humor\n"
...
int help_artsel()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("Thread/Subject/Article Selector Commands:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines(";	Enter article-scan mode.  If articles/subjects/threads were selected\n"
                           "	in the trn selector, article-scan will show only those articles.\n",
                           NOMARKING)) ||
        (cmd = print_lines("#	Read the current item only, temporarily ignoring all other selections.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("E	Toggle exclusion of non-selected items from the selection list.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("=	Toggle between the article and thread/subject selector.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("D	Mark unselected articles on the current page as read.  Start\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("P	Leave this group as-is and go on to the previous one.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("	entire articles, c to make it case-sensitive, r to scan read articles\n"
...
int help_multirc()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("The Newsrc Selector:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           "a-z,0-9	Select/deselect the indicated item by its letter or number.  Many of\n"
                           "	the alpha letters are omitted for the following commands.\n"
                           "SP	Perform the default command (usually > or Z).\n"
                           ...
                           ".	Toggle the current item's selection.\n",
                           NOMARKING)) ||
        (cmd = print_lines("-	Set a range, as in d - f.  Repeats the last marking action.\n"
                           "@	Toggle the selection of all visible items.\n",
                           NOMARKING)) ||
        (cmd = print_lines("E	Toggle exclusion of non-selected items from the selection list.\n"
                           "n, ]	Move down to the next item (try down-arrow also).\n"
                           ...
                           "^, $	Go to first/last page.\n",
                           NOMARKING)) ||
        (cmd = print_lines("R	Reverse the current sort order.\n", NOMARKING)) ||
        (cmd = print_lines("&	Start the option selector or set command-line switches.\n"
                           "&&	View or set macro definitions.\n"
                           "!cmd	Escape to a subshell.\n"
                           "H	This help message.\n"
...
int help_options()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("The Option Selector:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           "a-z,0-9	Select/deselect the indicated item by its letter or number.  Many of\n"
                           "	the alpha letters are omitted for the following commands.\n"
                           "SP	Perform the default command (usually > or Z).\n"
        ...
        (cmd = print_lines("R	Reverse the current sort order.\n"
                           ...
                           "&&	View or set macro definitions.\n",
                           NOMARKING)) ||
        (cmd = print_lines("!cmd	Escape to a subshell.\n"
                           "H	This help message.\n"
                           "h, ?	Enter online help browser.\n"
                           "q	Quit the option selector, abandoning changes.\n"
                           "\n",
                           NOMARKING)) ||
        (cmd = print_lines("	The options are as follows:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("News Selector Mode............ threads/subjects/articles\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Read Breadth First............ yes/no\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Auto Savename................. yes/no\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Erase Screen.................. yes/no\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Line Num for Goto............. [# line (1-n)]\n"
                           "Verify Input.................. yes/no\n"
...
int help_scanart()
{
    ...
    if ((cmd = print_lines("Article scan commands:\n", STANDOUT)) ||
        (cmd = print_lines("SP,CR	Display article and enter the Article Level.\n"
                           "	Note: Marked articles (see 'm') will be displayed first.\n"
                           "r	Read the pointed-to article first, then read marked articles.\n",
                           NOMARKING)) ||
        (cmd = print_lines("[0-9]	Jump to item number.  Enter a second digit or a command to jump\n"
                           "	to the item.\n"
                           "n, ]	Move down to the next item (try down-arrow also).\n"
                           "p, [	Move up to the previous item (try up-arrow also).\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("t	Move to top of current page.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("^L,^R	Redraw screen.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("m	Mark or unmark article for reading or a later command.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("s	Select (for zoom) this article.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("+	Enter thread selection mode (only if group is threaded).\n", NOMARKING)) ||
        (cmd = print_lines("!cmd	Escape to a subshell and execute cmd.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("/text	Search article descriptions forwards for text.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("f	Toggle subject following mode.  When enabled article threads\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("U	Toggle reading eligibility: read / unread+read\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("Scoring commands:\n", STANDOUT)) ||
        (cmd = print_lines("K(num)	Kill articles with scores equal to or below a number (num).\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("[Note: type ' or \" then RETURN for menus of scoring commands.]\n"
                           "\"\" <text>\n"
                           "	Adds <text> to the local (this newsgroup's) scorefile.\n"
                           "\"* <text>\n"
                           "	Adds <text> to the global scorefile.\n"
...
int help_univ()
{
    int cmd;
    page_start();
    if ((cmd = print_lines("The Universal Selector:\n", STANDOUT)) ||
        (cmd = print_lines("\n"
                           "a-z,0-9	Select/deselect the indicated item by its letter or number.  Many of\n"
                           "	the alpha letters are omitted for the following commands.\n"
                           "SP	Perform the default command (usually > or Z).\n"
        ...
        (cmd = print_lines("O	Set the selector's order.\n"
                           ...
                           NOMARKING)) ||
        (cmd = print_lines("^E	Edit the current universal configuration file.\n", NOMARKING)) ||
        (cmd = print_lines("&	Start the option selector or set command-line switches.\n"
                           "&&	View or set macro definitions.\n"
                           "!cmd	Escape to a subshell.\n"
                           "H	This help message.\n"
