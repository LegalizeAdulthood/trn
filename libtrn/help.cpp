...
struct MarkedPage
{
    ...
    const MarkedText *begin() const
    {
        return text;
    }
    const MarkedText *end() const
    {
        return text + size;
    }
};
...
static int print_lines(const MarkedPage &page)
{
    for (const MarkedText &text : page)
    {
        if (int cmd = print_lines(text.text, text.markup))
        {
            return cmd;
        }
    }
...
template <size_t N>
int print_lines(const std::array<MarkedText, N> &lines)
{
    return print_lines(MarkedPage{lines.data(), lines.size()});
}

static int print_page(const MarkedPage &page)
{
    page_start();
    return print_lines(page);
}

template <size_t N>
int print_page(const std::array<MarkedText, N> &page)
{
    return print_page(MarkedPage{page.data(), page.size()});
}

int help_page()
{
    constexpr std::array<MarkedText, 12> page{
        MarkedText{"Paging commands:\n", STANDOUT},
        {"\n"
        ...
        {"b       Back up one page.\n"
         ...
         "_C      Switch character set conversion.\n"},
...
int help_art()
{
    constexpr std::array<MarkedText, 17> page{
        MarkedText{"Article Selection commands:\n", STANDOUT},
        {"\n"
...
{
    page_start();
    if (int cmd = print_lines("Newsgroup Selection commands:\n", STANDOUT))
    {
        return cmd;
    }
    if (g_newsgroup_ptr)
    {
        constexpr std::array<MarkedText, 3> lines{
            MarkedText{"\n"
        ...
        if (const int cmd = print_lines(lines))
        {
            return cmd;
        }
    }
    constexpr std::array<MarkedText, 6> lines{
        MarkedText{"t       Toggle the newsgroup between threaded and unthreaded reading.\n"
                   "c       Catch up (mark all articles as read).\n"
...
{
    constexpr std::array<MarkedText, 9> page{
        MarkedText{"The Newsgroup Selector:\n", STANDOUT},
        {"\n"
...
{
    constexpr std::array<MarkedText, 7> page{
        MarkedText{"The Add-Newsgroups Selector:\n", STANDOUT},
        {"\n"
...
int help_subs()
{
    constexpr std::array<MarkedText, 14> page{
        MarkedText{"Valid substitutions are:\n", STANDOUT},
        {"\n"
...
{
    constexpr std::array<MarkedText, 10> page{
        MarkedText{"Thread/Subject/Article Selector Commands:\n", STANDOUT},
        {"\n"
...
int help_multirc()
{
    constexpr std::array<MarkedText, 6> page{
        MarkedText{"The Newsrc Selector:\n", STANDOUT},
        {"\n"
         "a-z,0-9 Select/deselect the indicated item by its letter or number.  Many of\n"
         "        the alpha letters are omitted for the following commands.\n"
         "SP      Perform the default command (usually > or Z).\n"
...
int help_options()
{
    constexpr std::array<MarkedText, 13> page{
        MarkedText{"The Option Selector:\n", STANDOUT},
        {"\n"
         "a-z,0-9 Select/deselect the indicated item by its letter or number.  Many of\n"
         "        the alpha letters are omitted for the following commands.\n"
         "SP      Perform the default command (usually > or Z).\n"
...
{
    constexpr std::array<MarkedText, 15> page{
        MarkedText{"Article scan commands:\n", STANDOUT},
...
{
    constexpr std::array<MarkedText, 7> page{
        MarkedText{"The Universal Selector:\n", STANDOUT},
        {"\n"
         "a-z,0-9 Select/deselect the indicated item by its letter or number.  Many of\n"
         "        the alpha letters are omitted for the following commands.\n"
         "SP      Perform the default command (usually > or Z).\n"
