...
do_group:
        ...
        case NG_NORM:
        case NG_SEL_NEXT:
            set_newsgroup(newsgroup_next(np));
            break;
        ...
        case NG_SEL_PRIOR:
            while ((np = newsgroup_prev(np)) != nullptr)
            {
                if (np->m_flags & NF_VISIT)
                {
                    goto do_group;
...
sel_restart:
    if (current_group_display_mode() != 's')
    {
        for (Newsrc *rp = g_multirc->m_first; rp; rp = rp->next)
        {
            ...
            {
                rp->data_source->find_group_desc("control");
                if (rp->data_source->m_desc_sf.m_fp)
                {
                    rp->data_source->m_flags |= DF_NO_XGTITLE; // TODO: ok?
                }
                else
                {
                    rp->data_source->m_desc_sf.m_refetch_secs = 0;
                }
            }
        }
    }
    ...
    if (s_sel_ret == '\r' || s_sel_ret == '\n' || s_sel_ret == 'Z' || s_sel_ret == '\t' || s_sel_ret == ';')
    {
        ...
        for (NewsgroupData *np = newsgroup_first(); np; np = newsgroup_next(np))
        {
            if ((np->m_flags & NF_INCLUDED) && (!g_selected_count || (np->m_flags & g_sel_mask)))
            {
                np->m_flags |= NF_VISIT;
            }
            else
